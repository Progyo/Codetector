...
class CodeGeeX2(BaseModel, GeneratorMixin, DetectorMixin):
    """
    Implementation of the CodeGeeX 2 LLM.
    """
    __MODEL_MAX_OUTPUT_LENGTH = 8192//4 * 3
    ...
    def getModelName(self) -> str:
        return 'codegeex2-6b-3cb3f8fa305c8188c6c997d0be2ccc4b87ba6f7f'
    ...
    def load(self, outputHiddenStates:bool=False) -> None:
        if not self.isLoaded():
            ...
            rev = '3cb3f8fa305c8188c6c997d0be2ccc4b87ba6f7f'
            ...
            self.__model = self.__AutoModel.from_pretrained(model_name_or_path,
    ...
    def toPromptFormat(self, sample:CodeSample):
        return '{}\n{}\n'.format(sample.toComment(' language: '+sample.getPL()), #{}\n
                                sample.toComment(' '+sample.prompt),)
                                # sample.toComment(' additional comments: do not include any other code besides the code described directly above. No yapping.'),)
            ...
            for i in range(self.__outputCount):
                ...
                for j in range(self.__bestOf):
                    tempOutput = self.__model.generate(inputs,
                                                    ...
                                                    pad_token_id=self.__tokenizer.special_tokens['<pad>'],
                                                    eos_token_id=self.__tokenizer.special_tokens['<eos>'],
                                                    bos_token_id=self.__tokenizer.special_tokens['<bos>'],
    ...
    def __getAttentionMask(self, input_ids):
        ...
        Else return `None`.
        """
        return input_ids.ne(self.__tokenizer.special_tokens['<pad>']).to(self.__model.device)
    ...
    def setMaxOutputLength(self, length:int) -> None:
        self.__outputLength = min(CodeGeeX2.__MODEL_MAX_OUTPUT_LENGTH, length)
    ...
    def __getLogits(self, sample:Sample, getHiddenStates:bool=False) -> tuple:
        inputs = self.__tokenizer(sample.content, return_tensors='pt',max_length=self.__MODEL_MAX_OUTPUT_LENGTH,truncation=True).to(self.__model.device)
